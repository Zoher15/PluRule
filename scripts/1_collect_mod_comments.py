...
def process_single_file(file_path: str) -> dict:
    ...
    if os.path.exists(output_file):
        print(f"Skipping {os.path.basename(input_file)} - output already exists")
...
def collect_subreddit_stats(results: list) -> dict:
    """Collect subreddit statistics from all processed files."""
    print("\nAggregating subreddit statistics...")
    ...
    print(f"Aggregated statistics for {len(total_subreddit_counts)} subreddits")
...
def main():
    ...
    print("Stage 1: Collecting Moderator Comments")
    print("=" * 40)
    ...
    print(f"Found {len(files)} RC files to process")
    print(f"Processing with {PROCESSES} parallel processes")
    print()
    ...
    subreddit_counts = collect_subreddit_stats(results)

    if not subreddit_counts:
        print("No subreddit statistics collected!")
        return 1
    ...
    print(f"Total mod comments: {total_comments:,}")
    print(f"Unique subreddits: {total_subreddits:,}")
    print(f"Rankings saved to: {rankings_file}")

    # Show top 10 subreddits
    print(f"\nTop 10 subreddits by mod comment count:")
    for entry in rankings_data['rankings'][:10]:
        print(f"  {entry['rank']:2d}. r/{entry['subreddit']}: {entry['mod_comment_count']:,}")
